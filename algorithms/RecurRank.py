...
class RecurRank(object):
    ...
    def run(self):
        ...
        while tau < self.T:
            ...
            # if tau == np.argmax(self.env.means):
            #     x, r = self.env.feedback(self.recommend_list)
            #     self.rewards[tau] = r
            # else:
            #     x, r = self.env.feedback(self.recommend_list)
            #     self.rewards[tau] = 0.1

            x, r = self.env.feedback(self.recommend_list)
            self.rewards[tau] = r

            # calculate whether selected target arm
            if target_arm in self.recommend_list:
                # print(self.recommend_list.index(target_arm))
                if x[self.recommend_list.index(target_arm)] == 1:
                    # print(1)
    ...
    def attack_run(self):
        ...
        while tau < self.T:
            ...
            if x.sum() != self.K:              
                ...
                if target_arm not in self.recommend_list: # target arm not got selected in K
                    one.append(1)
                    de_one_cost.append(de)
                    self.rewards[tau] -= de
                    x = np.array([0]*self.K) # set all other arm not pulled
                
                elif target_arm in self.recommend_list and  (self.recommend_list.index(target_arm) != 0): # target arm in K but not selected
                    two.append(1)
                    de_one_cost.append(de)
                    self.rewards[tau] -= de
                    x = np.array([0]*self.K)
                    x[self.recommend_list.index(target_arm)] = 1
                else:
                    three.append(1)
        ...
        print(sum(one),"\n", sum(two),"\n",sum(three))          
        return self.rewards, de_one_cost, num_t_click